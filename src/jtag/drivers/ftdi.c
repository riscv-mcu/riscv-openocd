...
#if BUILD_FTDI_CJTAG == 1
	} else if (oscan1_mode || jscan3_mode || nscan1_mode) {
...
static void cjtag_reset_online_activate(void)
{
	...
	if (!oscan1_mode && !jscan3_mode && !nscan1_mode)
		return; /* Nothing to do */

	if (oscan1_mode && jscan3_mode && nscan1_mode) {
		LOG_ERROR("Both oscan1_mode jscan3_mode nscan1_mode are \"on\". At most one of them can be enabled.");
		return;
	}
